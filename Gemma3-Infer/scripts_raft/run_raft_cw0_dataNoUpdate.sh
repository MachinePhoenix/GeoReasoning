...
CHECKPOINT_ROOT="/data/yanshi.xy/MLLM_RAFT/Gemma3-Infer/gemma_models_raft_cw0_dataNoUpdate"
DATA_ROOT="/data/yanshi.xy/LLaMA-Factory/data"
SCRIPTS_PATH="/data/yanshi.xy/MLLM_RAFT/Gemma3-Infer/scripts_raft"
cw=0.0
rw=1.0
...
for ((i=1; i<=$EPOCHS; i++)); do
    ...
    # # 输入数据路径
    data_name=geo_train_gemma3_$((i-1))_cw0_dataNoUpdate
    INPUT_DATA="$DATA_ROOT/$data_name.json"
    INTER_DATA="$DATA_ROOT/gemma3_inter_data${i}_cw0_dataNoUpdate.json"
    ...
    # # 最终输出数据路径
    OUTPUT_DATA="$DATA_ROOT/geo_train_gemma3_${i}_cw0_dataNoUpdate.json"
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python reasoning_rewarding_ray_noStatistics_dataNoUpdate.py \
        ...
        --project gemma_raft_cw0_dataNoUpdate \
        --current_epoch $i \
