...
SAMPLE_NUM=1
TOP_K=1
INITIAL_MODEL="/data/yanshi.xy/hf_models/Gemma3-4B"
CHECKPOINT_ROOT="/data/yanshi.xy/MLLM_RAFT/Gemma3-Infer/cold_start_fixUnderfitting"
DATA_ROOT="/data/yanshi.xy/LLaMA-Factory/data"
SCRIPTS_PATH="/data/yanshi.xy/MLLM_RAFT/Gemma3-Infer/scripts_eval"
cw=0.0
rw=1.0
...
for ((i=2; i<=$EPOCHS; i++)); do
    ...
    # # 输入数据路径
    data_name=geo_cold_start_gemma3_$((i-1))_cw0
    INPUT_DATA="$DATA_ROOT/$data_name.json"
    INTER_DATA="$DATA_ROOT/geo_cold_start_inter_gemma3_${i}_cw0.json"
    
    # Caption Generation
    cd /data/yanshi.xy/LLaMA-Factory
    # export RAY_memory_monitor_refresh_ms=0
    export RAY_memory_usage_threshold=1.0
    CUDA_VISIBLE_DEVICES=4,5,6,7 python scripts/caption_generation_sample1_llamafactory_ray.py \
    ...
    # # 最终输出数据路径
    OUTPUT_DATA="$DATA_ROOT/geo_cold_start_gemma3_${i}_cw0.json"
    
    # Reasoning Reward
    cd /data/yanshi.xy/MLLM_RAFT/Gemma3-Infer/src_raft
    CUDA_VISIBLE_DEVICES=4,5,6,7 python reasoning_rewarding_ray_noStatistics_dataNoUpdate.py \
    ...
    #     dataset=geo_train_gemma3_${i}_cw0 \
    #     num_train_epochs=1 \
