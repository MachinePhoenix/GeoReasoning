...
CHECKPOINT_ROOT="/data/yanshi.xy/MLLM_RAFT/Qwen2.5-VL-Finetune/qwen_models_raft_cw0"
...
for ((i=1; i<=$EPOCHS; i++)); do
    ...
    # # 输入数据路径
    INPUT_DATA="$DATA_ROOT/data$((i-1))_cw0.json"
    
    INTER_DATA="$DATA_ROOT/inter_data${i}_cw0.json"
    
    # Caption Generation
    cd /data/yanshi.xy/MLLM_RAFT/Qwen2.5-VL-Finetune/src_raft
    CUDA_VISIBLE_DEVICES=4,5,6,7 python caption_generation_ray.py \
    ...
    # # 最终输出数据路径
    OUTPUT_DATA="$DATA_ROOT/data${i}_cw0.json"
    
    # Reasoning Reward
    cd /data/yanshi.xy/MLLM_RAFT/Qwen2.5-VL-Finetune/src_raft
    CUDA_VISIBLE_DEVICES=4,5,6,7 python reasoning_rewarding_ray_noStatistics.py \
    ...
    CUDA_VISIBLE_DEVICES=4,5,6,7 deepspeed src/train/train_sft.py \
        ...
        --epoch=$i \
        --project_name="RAFT_SFT_epoch${i}_cw0"
