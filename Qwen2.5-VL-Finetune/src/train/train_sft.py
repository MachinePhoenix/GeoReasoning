...
from trainer_this import QwenTrainer
...
import pdb
import wandb
# wandb.init(project='xinyue_machine', entity='xinyue_machine')
...
def train(epoch, project_name):
    global local_rank

    hf_parser = HfArgumentParser(
        (ModelArguments, DataArguments, TrainingArguments))
    
    model_args, data_args, training_args = hf_parser.parse_args_into_dataclasses(args=remaining_args)
    # training_args.save_strategy = "no"
    if training_args.local_rank == 0 or training_args.local_rank == -1:
        wandb.init(
            project="qwen_cold_start",
            name=project_name,
            config={
                "epoch": epoch,
    ...
    #     trainer.train()
    trainer.train()
    # trainer.save_model(output_dir=f"checkpoint-epoch{epoch}")
    trainer.save_state()
...
if __name__ == "__main__":
    ...
    train(args.epoch, args.project_name)