...
SAMPLE_NUM=1
TOP_K=1
INITIAL_MODEL="/data/yanshi.xy/hf_models/Qwen2.5-VL-3B-Instruct"
CHECKPOINT_ROOT="/data/yanshi.xy/MLLM_RAFT/Qwen2.5-VL-Finetune/cold_start_fixUnderfitting"
DATA_ROOT="/data/yanshi.xy/MLLM_RAFT/Qwen2.5-VL-Finetune/GeoReasoning/train"
SCRIPTS_PATH="/data/yanshi.xy/MLLM_RAFT/Qwen2.5-VL-Finetune/scripts_eval"
cw=0.0
rw=1.0
...
for ((i=2; i<=$EPOCHS; i++)); do
    ...
    if [ $i -eq 0 ]; then
        ...
        CURRENT_MODEL="$CHECKPOINT_ROOT/checkpoint-epoch$i"
    fi

    echo "[1/2] Rollout Phase - Caption Generation:"
    # # 输入数据路径
    INPUT_DATA="$DATA_ROOT/cold_start_data$((i-1))_cw0.json"
    
    INTER_DATA="$DATA_ROOT/cold_start_inter_data${i}_cw0.json"
    
    # Caption Generation
    cd /data/yanshi.xy/MLLM_RAFT/Qwen2.5-VL-Finetune/src_eval
    CUDA_VISIBLE_DEVICES=0,1,2,3 python caption_generation_sample1_ray.py \
    ...
    echo "[2/2] Rollout Phase - Reasoning Reward:"
    # 最终输出数据路径
    OUTPUT_DATA="$DATA_ROOT/cold_start_data${i}_cw0.json"
    
    # Reasoning Reward
    cd /data/yanshi.xy/MLLM_RAFT/Qwen2.5-VL-Finetune/src_raft
    CUDA_VISIBLE_DEVICES=0,1,2,3 python reasoning_rewarding_ray_noStatistics_dataNoUpdate.py \
